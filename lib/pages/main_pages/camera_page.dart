...
class _CameraScreenState extends State<CameraScreen> {
  ...
  Future<void> _analyzeFoodImage(String imagePath) async {
    ...
    try {
      ...
      final Map<String, dynamic> analysisResult = await apiService.analyzeFoodImage(imageFile);
    ...
    try {
      ...
      for (var item in jsonList) {
        if (item is Map<String, dynamic>) {
          ...
          String mealName =
              item['mealorfood_name'] ?? item['mealName'] ?? item['name'] ?? 'Unknown Food';

          String servingSize =
              item['serving_size'] ?? item['servingSize'] ?? item['serving'] ?? '1 serving';
  ...
  Future<void> _saveFoodToLog(FoodItem food) async {
    try {
      ...
      if (user != null) {
        ...
        await FirebaseFirestore.instance.collection('food_logs').doc(foodLogId).set(foodLogData);
                    ...
                    child: Stack(
                      ...
                      children: [
                        FloatingActionButton(
                          ...
                          backgroundColor: _isProcessing ? Colors.grey : AppColors.primaryText,
                          child: _isProcessing
                              ? CircularProgressIndicator(
                          ...
                          child: GestureDetector(
                            onTap: _isProcessing
                                ...
                                : () async {
                                    ...
                                    try {
                                      final GeminiApiService apiService = GeminiApiService();
                                      final result = await apiService.analyzeImageFromGallery();

                                      if (result != null && mounted) {
                                        debugPrint('Gallery analysis result: $result');
